...
public function state_search($search,$conn){
 ...
 public function search_address($search,$conn){
    ...
    public function get_price_history ($usertype,$conn){
      $query="SELECT BusinessName,Address,Contact,Symbol,Status, Cost,State,City,Name,Date,Time FROM view_price_history WHERE Userid='$usertype' AND Date >= DATE_SUB(NOW(), INTERVAL 7 DAY) ORDER BY Date, time ASC";
      $product=$conn->query($query); 
        if($product->num_rows>0){  
            while($row= $product->fetch_assoc()){
            $result[] =  $row;
